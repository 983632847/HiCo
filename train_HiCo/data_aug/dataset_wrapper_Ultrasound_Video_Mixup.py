...
class USDataset_video(Dataset):
    ...
    def __init__(self, data_dir, transform=None, LabelList=None, DataList=None, Checkpoint_Num=None):
        ...
        # self.label_name = {"Cov": 0, "Non": 1}  # COVID_Xray_5k
    ...
    def __getitem__(self, index):
        ...
        if len(path_imgs) >= 3:  # more than 3 images in one video
            ...
            if index in self.LabelList:
                # path_imgs[0]: '/home/zhangchunhui/MedicalAI/Dataset/Butte/Cov-Cardiomyopathy_mp4'
                # path_imgs[0][43:46]: 'Cov'

                label1 = self.label_name[path_imgs[0][41:44]]
                label2 = self.label_name[path_imgs[1][41:44]]
                label3 = self.label_name[path_imgs[2][41:44]]
            else:
            ...
            if index in self.LabelList:
                label1 = self.label_name[path_imgs[0][41:44]]
                label2 = self.label_name[path_imgs[0][41:44]]
            else:
                label1, label2 = 9999, 9999  # unlabel data = 9999
...
class USDataset_image(Dataset):
    ...
    def __getitem__(self, index):
        ...
        label1 = 0 if path_img[0].lower()[40:].find("cov") > -1 else (1 if path_img[0].lower()[64:].find("pneu") > -1 else 2)
...
class DataSetWrapper(object):
    ...
    def get_data_loaders(self):
        ...
        if use_video:
            ...
            # US-4
            train_dataset = USDataset_video("/Data/Medical_Image_Datasets/train/US_4d/",
                                            transform=SimCLRDataTransform(data_augment), LabelList=self.LabelList, DataList=self.DataList)  # augmented from 2 images

            # 1 video-CLUST
            # train_dataset = USDataset_video("/Data/Medical_Image_Datasets/train/CLUST/",
            #                                 transform=SimCLRDataTransform(data_augment), LabelList=self.LabelList, DataList=self.DataList)  # augmented from 2 images
            # 1 video-Liver
            # train_dataset = USDataset_video("/Data/Medical_Image_Datasets/train/Liver/",
            #                                 transform=SimCLRDataTransform(data_augment), LabelList=self.LabelList, DataList=self.DataList)  # augmented from 2 images
            # 1 video-COVID
            # train_dataset = USDataset_video("/Data/Medical_Image_Datasets/train/COVID/",
            #                                 transform=SimCLRDataTransform(data_augment), LabelList=self.LabelList, DataList=self.DataList)  # augmented from 2 images
            # 1 video-Butte
            # train_dataset = USDataset_video("/Data/Medical_Image_Datasets/train/Butte/",
            #                                 transform=SimCLRDataTransform(data_augment), LabelList=self.LabelList, DataList=self.DataList)  # augmented from 2 images
        else:
            print('\nDo not use video augmentation!')
            # Images
            # train_dataset = USDataset_image("/Data/Medical_Image_Datasets/train/Butte/",
