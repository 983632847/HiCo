...
class RandomResizedCropCoord(object):
    ...
    def __call__(self, img):
        ...
        # ori_img = F.resize(img, self.size, self.interpolation) # 224x224
        # ori_img = np.transpose(ori_img, (2,0,1))  # (h,w,c) --> (c,h,w)
        # ori_img = torch.Tensor(np.array(ori_img))
        # return ori_img, F.resized_crop(img, i, j, h, w, self.size, self.interpolation), coord
